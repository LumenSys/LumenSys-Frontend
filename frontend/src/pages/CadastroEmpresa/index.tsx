...
type CompanyData = {
  cpfCnpj: string;
  companyName: string; // Alterado de 'name' para 'companyName'
  tradeName: string;
  email: string;
  phone: string;
  street: string;
  ...
  neighborhood: string;
  city: string;
  uf: string; // Será convertido para UF maiúsculo
  companyLogo?: File | null; // Alterado de 'logo' para 'companyLogo'
};

const initialFields: CompanyData = {
  cpfCnpj: "",
  companyName: "", // Alterado
  tradeName: "",
  email: "",
  phone: "",
  street: "",
  ...
  city: "",
  uf: "",
  companyLogo: null, // Alterado
};

// Componente para exibir logo da empresa
interface CompanyLogoDisplayProps {
  companyLogoBytes?: string; // Base64 string recebida do backend
  companyName: string;
  size?: 'sm' | 'md' | 'lg';
  className?: string;
}
...
const CompanyLogoDisplay: React.FC<CompanyLogoDisplayProps> = ({ 
  ...
  if (companyLogoBytes) {
    // Se recebido do backend, já vem como Base64
...
const CadastroEmpresa = () => {
  ...
  // Função para converter File para ArrayBuffer e depois para Base64
  ...
  const validateForm = () => {
    ...
    const requiredFields: (keyof CompanyData)[] = [
      'cpfCnpj', 'companyName', 'tradeName', 'email', 'phone', 
  ...
  const getFieldLabel = (key: keyof CompanyData): string => {
    const labels: Record<keyof CompanyData, string> = {
      cpfCnpj: 'CNPJ',
      companyName: 'Razão Social', // Alterado
      ...
      city: 'Cidade',
      uf: 'UF',
      companyLogo: 'Logo da Empresa' // Alterado
    };
    return labels[key];
  };
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      setFields(prev => ({ 
        ...prev, 
        companyLogo: file // Alterado de 'logo' para 'companyLogo'
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Preparar dados da empresa para o backend C#
      const companyData: any = {
        cpfCnpj: fields.cpfCnpj.replace(/\D/g, ''),
        companyName: fields.companyName, // Alterado de 'name'
        ...
        city: fields.city,
        uf: fields.uf.toUpperCase(), // Garantir que seja maiúsculo
        companyLogo: null // Será preenchido abaixo se houver imagem
      };

      // Converter imagem para Base64 se existir
      if (fields.companyLogo) {
        const base64Logo = await fileToBase64(fields.companyLogo);
        companyData.companyLogo = base64Logo;
      }
      ...
      Cookies.set('lastCompanyRegistered', JSON.stringify({
        companyName: fields.companyName, // Alterado
        tradeName: fields.tradeName,
  ...
  const loadCompanyImage = async (companyId: string) => {
    try {
      ...
      if (company.companyLogo) {
        // Se vier como Base64 do backend
        const imageUrl = `data:image/jpeg;base64,${company.companyLogo}`;
        ...
        <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <InputField
                label="Razão Social"
                name="companyName" // Alterado de 'name' para 'companyName'
                placeholder="Nome oficial da empresa"
                value={fields.companyName} // Alterado
                onChange={handleChange}
                onBlur={handleBlur}
                required
                className="focus:ring-primary text-textPrimary bg-background"
              />
              {isEmpty("companyName") && ( // Alterado
                <span className="text-danger text-sm mt-1 block">Campo obrigatório</span>
              )}
            </div>
